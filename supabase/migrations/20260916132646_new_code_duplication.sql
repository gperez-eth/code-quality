-- Duplication becomes a new-code measure.
--
-- The comment this migration deletes said duplication "cannot be attributed to
-- new code". Per line that is true; per file it is not, and per file is the
-- granularity file_measures already stores. Paired with the analyzer dropping
-- its duplication threshold from ten significant lines to five, following the
-- GitClear research: at ten, this repository reported 0.00% duplication across
-- 3820 lines of packages/, which is a threshold saying nothing at all.
--
-- Everything else in import_analysis is byte-identical to
-- 20260916021000_vault_tokens.sql. Verify with a diff before trusting this.
